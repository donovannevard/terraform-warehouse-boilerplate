...
variable "redshift_inbound_cidr_restriction" {
  description = "CIDR allowed to reach the Redshift cluster on port 5439 (e.g. your office/VPN range). No default — pick a deliberate value, do not leave this open to 0.0.0.0/0."
  type        = string

  validation {
    condition     = can(cidrhost(var.redshift_inbound_cidr_restriction, 0))
    error_message = "redshift_inbound_cidr_restriction must be a valid CIDR block."
  }
}
